...
import { handleDeleteNhaxuong, handleDeleteNhaxuongMany, handleExportNhaxuong } from '@/app/(main)/quan-tri/csvc/nha-xuong/actions';
import NhaxuongModal from '@/components/csvc/nhaxuong/modal';
import ModalImportNhaxuong from '@/components/csvc/nhaxuong/modal.import';
import NhaxuongDetail from '@/components/csvc/nhaxuong/detail';
...
const TableNhaxuong = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteNhaxuong(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/nha-xuong?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteNhaxuongMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportNhaxuong(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'nha-xuong.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích nhà xưởng:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            >
                <NhaxuongDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TableNhaxuong;
