...
import { handleDeletePgsgs, handleDeletePgsgsMany, handleExportPgsgs } from '@/app/(main)/quan-tri/csvc/pgs-gs/actions';
import PgsgsModal from '@/components/csvc/pgsgs/modal';
import ModalImportPgsgs from '@/components/csvc/pgsgs/modal.import';
import PgsgsDetail from '@/components/csvc/pgsgs/detail';
...
const TablePgsgs = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeletePgsgs(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/pgs-gs?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeletePgsgsMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportPgsgs(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'Pgs-gs.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex wrap style={{ marginBottom: 16, gap: 8 }} justify="space-between" align="center">
                <h2>Danh sách phòng phó giáo sư - giáo sư</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích phòng phó giáo sư - giáo sư:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            />
            <Drawer
                title="Chi tiết phòng phó giáo sư - giáo sư"
            ...
            >
                <PgsgsDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TablePgsgs;
