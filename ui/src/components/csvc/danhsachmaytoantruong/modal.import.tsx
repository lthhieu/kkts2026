...
import { handleCreateManyMaytoantruong } from '@/app/(main)/quan-tri/csvc/danh-sach-may-toan-truong/actions';
...
const ModalImportMaytoantruong = (props: IProps) => {
                ...
                try {
                    ...
                    worksheet.eachRow((row, rowNumber) => {
                        ...
                        const obj = {
                            ...
                            des: getVal(2) ? String(getVal(2)) : '',
                            unit: getVal(3) ? String(getVal(3)) : null,
                            room: getVal(4) ? String(getVal(4)) : null,
                            nam_sd: Number(getVal(5)) || 0,
                            sl: Number(getVal(6)) || 0,
                            nguyengia: Number(getVal(7)) || 0,
                            cate: getVal(8) ? String(getVal(8)) : '',
                        };
    ...
    const handleOk = () => {
        ...
        setTimeout(async () => {
            const response = await handleCreateManyMaytoantruong(dataImport, access_token ?? '');
            if (response.data) {
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal
                title="Import dữ liệu tài sản cố định"
            ...
            >
                <Dragger {...uploadProps}>
                    ...
                    <p className="ant-upload-hint">
                        Chỉ hỗ trợ file excel và csv <a onClick={(e) => e.stopPropagation()} href={`${process.env.NEXT_PUBLIC_FRONTEND_URI}sample/sample-maytoantruong.xlsx`} download>Tải file mẫu</a>
                    </p>
                </Dragger>
                <div>
                    <Table<any>
                        ...
                        columns={[
                            { dataIndex: 'name', title: 'Tên TSCĐ' },
                            { dataIndex: 'des', title: 'Mã số / Mô tả' },
                            { dataIndex: 'unit', title: 'Đơn vị' },
                            { dataIndex: 'room', title: 'Phòng' },
                            { dataIndex: 'nam_sd', title: 'Năm sử dụng' },
                            { dataIndex: 'sl', title: 'Số lượng' },
                            { dataIndex: 'nguyengia', title: 'Nguyên giá' },
                        ]}
                    />
                </div>
            </Modal>
...
export default ModalImportMaytoantruong;
