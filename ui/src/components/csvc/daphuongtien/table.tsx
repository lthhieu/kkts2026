...
import { handleDeleteDaphuongtien, handleDeleteDaphuongtienMany, handleExportDaphuongtien } from '@/app/(main)/quan-tri/csvc/da-phuong-tien/actions';
import DaphuongtienModal from '@/components/csvc/daphuongtien/modal';
import ModalImportDaphuongtien from '@/components/csvc/daphuongtien/modal.import';
import DaphuongtienDetail from '@/components/csvc/daphuongtien/detail';
...
const TableDaphuongtien = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteDaphuongtien(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/da-phuong-tien?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteDaphuongtienMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportDaphuongtien(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'da-phuong-tien.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex wrap style={{ marginBottom: 16, gap: 8 }} justify="space-between" align="center">
                <h2>Danh sách phòng đa phương tiện</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích phòng đa phương tiện:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            />
            <ModalImportDaphuongtien
                access_token={access_token}
                isModalImportOpen={isModalImportOpen}
                setIsModalImportOpen={setIsModalImportOpen}
            />
            <Drawer
                title="Chi tiết phòng đa phương tiện"
            ...
            >
                <DaphuongtienDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TableDaphuongtien;
