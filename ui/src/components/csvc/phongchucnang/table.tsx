...
interface IProps {
    ...
    user: IUser | null;
    summary: any[] | null;
    type: ILoaiphong[] | null; // Add this line
}

const Context = React.createContext({ name: 'Default' });
const { Text } = Typography;

const TablePhongchucnang = (props: IProps) => {
    const { data, access_token, meta, user, summary, type } = props;
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isModalImportOpen, setIsModalImportOpen] = useState(false);
    const [status, setStatus] = useState('');
    const [loading, setLoading] = useState(false);
    const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
    const [dataUpdate, setDataUpdate] = useState<null | IPhongchucnang>(null);
    const [selectedRecord, setSelectedRecord] = useState<null | IPhongchucnang>(null);
    ...
    const showDrawer = (record: IPhongchucnang) => { setSelectedRecord(record); setOpenDrawer(true); };
    const onClose = () => setOpenDrawer(false);

    const deleteItem = async (_id: string) => {
        const res = await handleDeletePhongchucnang(_id, access_token);
    ...
    const columns: TableProps<IPhongchucnang>['columns'] = [
        {
            title: 'Mã phòng',
            dataIndex: 'ma',
            key: 'ma',
        ...
        },
        {
            ...
            dataIndex: ['type', 'name'],
            key: 'type',
            render: (_, record) => <Typography.Text copyable={{ text: record.type?._id || "", tooltips: 'Sao chép' }}>{record.type?.name}</Typography.Text>
        },
        {
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeletePhongchucnangMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích xây dựng phòng chức năng:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<IPhongchucnang>
            ...
            <PhongchucnangModal
                ...
                types={type}
            />
            <ModalImportPhongchucnang
                access_token={access_token}
                isModalImportOpen={isModalImportOpen}
                setIsModalImportOpen={setIsModalImportOpen}
            />
            <Drawer
                title="Chi tiết phòng - chức măng"
            ...
            >
                <PhongchucnangDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TablePhongchucnang;
