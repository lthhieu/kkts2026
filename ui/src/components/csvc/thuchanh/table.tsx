...
import { handleDeleteThuchanh, handleDeleteThuchanhMany, handleExportThuchanh } from '@/app/(main)/quan-tri/csvc/thuc-hanh/actions';
import ThuchanhModal from '@/components/csvc/thuchanh/modal';
import ModalImportThuchanh from '@/components/csvc/thuchanh/modal.import';
import ThuchanhDetail from '@/components/csvc/thuchanh/detail';
...
const TableThuchanh = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteThuchanh(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/thuc-hanh?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteThuchanhMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportThuchanh(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'thuc-hanh.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích phòng thực hành:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            />
            <Drawer
                title="Chi tiết phòng thực hành"
            ...
            >
                <ThuchanhDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TableThuchanh;
