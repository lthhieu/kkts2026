...
import { handleDeleteHoitruong, handleDeleteHoitruongMany, handleExportHoitruong } from '@/app/(main)/quan-tri/csvc/hoi-truong/actions';
import HoitruongDetail from '@/components/csvc/hoitruong/detail';
import ModalImportHoitruong from '@/components/csvc/hoitruong/modal.import';
import HoitruongModal from '@/components/csvc/hoitruong/modal';
...
const TableHoitruong = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteHoitruong(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/hoi-truong?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteHoitruongMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportHoitruong(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'hoi-truong.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích hội trường:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            />
            <ModalImportHoitruong
                access_token={access_token}
                isModalImportOpen={isModalImportOpen}
                setIsModalImportOpen={setIsModalImportOpen}
            />
            <Drawer
                title="Chi tiết hội trường"
            ...
            >
                <HoitruongDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TableHoitruong;
