...
import { handleCreateManyTbiptn } from '@/app/(main)/quan-tri/csvc/tbiptn/actions';
...
const ModalImportTbiptn = (props: IProps) => {
            ...
            if (status === 'done' || info.fileList.length > 0) {
                try {
                    ...
                    worksheet.eachRow((row, rowNumber) => {
                        ...
                        // A=ma_tb, B=ten_tb, C=ma_ct_csvc(id), D=nam_sx, E=hang_sx
    ...
    const handleOk = () => {
        ...
        setTimeout(async () => {
            const response = await handleCreateManyTbiptn(dataImport, access_token ?? '');
            if (response.data) {
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal
                title="Import dữ liệu thiết bị PTN"
            ...
            >
                <Dragger {...uploadProps}>
                    ...
                    <p className="ant-upload-hint">
                        ...
                        &nbsp;<a onClick={(e) => e.stopPropagation()} href={`${process.env.NEXT_PUBLIC_FRONTEND_URI}sample/sample-tbiptn.xlsx`} download>Tải file mẫu</a>
                    </p>
                </Dragger>
                <div>
                    <Table<any>
                        ...
                        columns={[
                            { dataIndex: 'ma_tb', title: 'Mã thiết bị' },
                            { dataIndex: 'ten_tb', title: 'Tên thiết bị' },
                            { dataIndex: 'hang_sx', title: 'Hãng sản xuất' },
                            { dataIndex: 'nam_sx', title: 'Năm sản xuất' },
...
export default ModalImportTbiptn;
