...
import { handleDeleteThinghiem, handleDeleteThinghiemMany, handleExportThinghiem } from '@/app/(main)/quan-tri/csvc/thi-nghiem/actions';
import ThinghiemModal from '@/components/csvc/thinghiem/modal';
import ModalImportThinghiem from '@/components/csvc/thinghiem/modal.import';
import ThinghiemDetail from '@/components/csvc/thinghiem/detail';
...
const TableThinghiem = (props: IProps) => {
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteThinghiem(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/thi-nghiem?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteThinghiemMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const handleExport = async () => {
        const blob = await handleExportThinghiem(access_token);

        const url = window.URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = 'thi-nghiem.csv';
        a.click();
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 24, marginBottom: 12 }}>
                <Text strong>- Tổng diện tích phòng thí nghiệm:  {summary?.[0]?.totalDT?.toLocaleString('vi-VN')} m²</Text>
            </div>
            <Table<ICsvcSubject>
            ...
            />
            <ModalImportThinghiem
                access_token={access_token}
                isModalImportOpen={isModalImportOpen}
                setIsModalImportOpen={setIsModalImportOpen}
            />
            <Drawer
                title="Chi tiết phòng thí nghiệm"
            ...
            >
                <ThinghiemDetail data={selectedRecord} />
            </Drawer>
        </Context.Provider>
    );
};

export default TableThinghiem;
