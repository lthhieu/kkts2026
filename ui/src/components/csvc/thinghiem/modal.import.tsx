...
import { handleCreateManyThinghiem } from '@/app/(main)/quan-tri/csvc/thi-nghiem/actions';
...
const ModalImportThinghiem = (props: IProps) => {
    ...
    const handleOk = () => {
        ...
        setTimeout(async () => {
            const response = await handleCreateManyThinghiem(dataImport, access_token ?? '');
            if (response.data) {
            ...
            >
                <Dragger {...uploadProps}>
                    ...
                    <p className="ant-upload-hint">
                        Chỉ hỗ trợ file excel và csv <a onClick={(e) => e.stopPropagation()} href={`${process.env.NEXT_PUBLIC_FRONTEND_URI}sample/sample-thinghiem.xlsx`} download>Tải file mẫu</a>
                    </p>
                </Dragger>
                <div>
                    <Table<any>
...
export default ModalImportThinghiem;
