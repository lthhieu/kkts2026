...
import { handleCreateManyCholamvieccuagv } from '@/app/(main)/quan-tri/csvc/cho-lam-viec-cua-giang-vien/actions';
...
const ModalImportCholamvieccuagv = (props: IProps) => {
    ...
    const handleOk = () => {
        ...
        setTimeout(async () => {
            const response = await handleCreateManyCholamvieccuagv(dataImport, access_token ?? '');
            if (response.data) {
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal
                title="Import dữ liệu chỗ làm việc của giảng viên"
            ...
            >
                <Dragger {...uploadProps}>
                    ...
                    <p className="ant-upload-hint">
                        Chỉ hỗ trợ file excel và csv <a onClick={(e) => e.stopPropagation()} href={`${process.env.NEXT_PUBLIC_FRONTEND_URI}sample/sample-cholamvieccuagv.xlsx`} download>Tải file mẫu</a>
                    </p>
                </Dragger>
                <div>
                    <Table<any>
...
export default ModalImportCholamvieccuagv;
