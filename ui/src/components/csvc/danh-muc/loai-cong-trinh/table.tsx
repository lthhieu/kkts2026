...
import { handleDeleteLoaicongtrinhcsvc, handleDeleteLoaicongtrinhcsvcMany } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-cong-trinh/actions';
import LoaicongtrinhcsvcModal from '@/components/csvc/danh-muc/loai-cong-trinh/modal';
import ModalImport from '@/components/csvc/danh-muc/loai-cong-trinh/modal.import';
...
const TableLoaicongtrinhcsvc = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ILoaicongtrinhcsvc>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/loai-cong-trinh?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteLoaicongtrinhcsvcMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ILoaicongtrinhcsvc> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="loai-cong-trinh.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <LoaicongtrinhcsvcModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableLoaicongtrinhcsvc;
