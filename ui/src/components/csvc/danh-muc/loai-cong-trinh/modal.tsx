...
import { handleCreateOrUpdateLoaicongtrinhcsvc } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-cong-trinh/actions';
...
interface IProps {
    ...
    dataUpdate: null | ILoaicongtrinhcsvc;
    setDataUpdate: (value: null | ILoaicongtrinhcsvc) => void;
}

const Context = React.createContext({ name: 'Default' });

const LoaicongtrinhcsvcModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateLoaicongtrinhcsvc(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm loại công trình' : 'Cập nhật loại công trình'}
...
export default LoaicongtrinhcsvcModal;
