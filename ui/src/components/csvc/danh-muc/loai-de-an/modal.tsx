...
import { handleCreateOrUpdateLoaidean } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-de-an/actions';
...
interface IProps {
    ...
    dataUpdate: null | ILoaidean;
    setDataUpdate: (value: null | ILoaidean) => void;
}

const Context = React.createContext({ name: 'Default' });

const LoaideanModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateLoaidean(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm loại đề án' : 'Cập nhật loại đề án'}
