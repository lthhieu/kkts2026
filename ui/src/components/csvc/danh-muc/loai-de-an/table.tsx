...
import { handleDeleteLoaidean, handleDeleteLoaideanMany } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-de-an/actions';
import LoaideanModal from '@/components/csvc/danh-muc/loai-de-an/modal';
import ModalImport from '@/components/csvc/danh-muc/loai-de-an/modal.import';
...
const TableLoaidean = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ILoaidean>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/loai-de-an?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteLoaideanMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ILoaidean> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="loai-de-an.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <LoaideanModal setStatus={setStatus} status={status} access_token={access_token}
