...
import { handleDeletePhanloai, handleDeletePhanloaiMany } from '@/app/(main)/quan-tri/csvc/danh-muc/phan-loai/actions';
import PhanloaiModal from '@/components/csvc/danh-muc/phan-loai/modal';
import ModalImport from '@/components/csvc/danh-muc/phan-loai/modal.import';
...
const TablePhanloai = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | IPhanloai>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/phan-loai?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeletePhanloaiMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<IPhanloai> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="phan-loai.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <PhanloaiModal setStatus={setStatus} status={status} access_token={access_token}
