...
import { handleCreateOrUpdatePhanloai } from '@/app/(main)/quan-tri/csvc/danh-muc/phan-loai/actions';
...
interface IProps {
    ...
    dataUpdate: null | IPhanloai;
    setDataUpdate: (value: null | IPhanloai) => void;
}

const Context = React.createContext({ name: 'Default' });

const PhanloaiModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdatePhanloai(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm phân loại' : 'Cập nhật phân loại'}
