...
import { handleCreateOrUpdateMucdichsudungcsvc } from '@/app/(main)/quan-tri/csvc/danh-muc/muc-dich-sd-csvc/actions';
...
interface IProps {
    ...
    dataUpdate: null | IMucdichsudungcsvc;
    setDataUpdate: (value: null | IMucdichsudungcsvc) => void;
}

const Context = React.createContext({ name: 'Default' });

const MucdichsudungcsvcModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateMucdichsudungcsvc(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm mục đích sử dụng CSVC' : 'Cập nhật mục đích sử dụng CSVC'}
...
export default MucdichsudungcsvcModal;
