...
import { handleDeleteMucdichsudungcsvc, handleDeleteMucdichsudungcsvcMany } from '@/app/(main)/quan-tri/csvc/danh-muc/muc-dich-sd-csvc/actions';
import MucdichsudungcsvcModal from '@/components/csvc/danh-muc/muc-dich-sd-csvc/modal';
import ModalImport from '@/components/csvc/danh-muc/muc-dich-sd-csvc/modal.import';
...
const TableMucdichsudungcsvc = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | IMucdichsudungcsvc>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/muc-dich-sd-csvc?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteMucdichsudungcsvcMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<IMucdichsudungcsvc> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách mục đích sử dụng CSVC</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="muc-dich-sd-csvc.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <MucdichsudungcsvcModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableMucdichsudungcsvc;
