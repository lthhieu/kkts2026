...
import { handleDeleteTinhthanhpho, handleDeleteTinhthanhphoMany } from '@/app/(main)/quan-tri/csvc/danh-muc/tinh-thanh-pho/actions';
import TinhthanhphoModal from '@/components/csvc/danh-muc/tinh-thanh-pho/modal';
import ModalImport from '@/components/csvc/danh-muc/tinh-thanh-pho/modal.import';
...
const TableTinhthanhpho = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ITinhthanhpho>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/tinh-thanh-pho?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteTinhthanhphoMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ITinhthanhpho> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="tinh-thanh-pho.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <TinhthanhphoModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableTinhthanhpho;
