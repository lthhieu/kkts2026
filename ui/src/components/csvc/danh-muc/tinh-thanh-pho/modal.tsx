...
import { handleCreateOrUpdateTinhthanhpho } from '@/app/(main)/quan-tri/csvc/danh-muc/tinh-thanh-pho/actions';
...
interface IProps {
    ...
    dataUpdate: null | ITinhthanhpho;
    setDataUpdate: (value: null | ITinhthanhpho) => void;
}

const Context = React.createContext({ name: 'Default' });

const TinhthanhphoModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateTinhthanhpho(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm tỉnh thành phố' : 'Cập nhật tỉnh thành phố'}
...
export default TinhthanhphoModal;
