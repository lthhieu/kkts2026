...
import { handleDeleteCountries, handleDeleteCountriesMany } from '@/app/(main)/quan-tri/csvc/danh-muc/quoc-gia/actions';
import CountryModal from '@/components/csvc/danh-muc/quoc-gia/modal';
import ModalImport from '@/components/csvc/danh-muc/quoc-gia/modal.import';
...
const TableCountry = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ICountry>(null);
    ...
    const deleteItem = async (_id: string) => {
        const res = await handleDeleteCountries(_id, access_token);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/quoc-gia?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteCountriesMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ICountry> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="quoc-gia.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <CountryModal setStatus={setStatus} status={status} access_token={access_token}
