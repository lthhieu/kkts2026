...
import { handleCreateOrUpdateCountries } from '@/app/(main)/quan-tri/csvc/danh-muc/quoc-gia/actions';
...
interface IProps {
    ...
    dataUpdate: null | ICountry;
    setDataUpdate: (value: null | ICountry) => void;
}

const Context = React.createContext({ name: 'Default' });

const CountryModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateCountries(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm quốc gia' : 'Cập nhật quốc gia'}
