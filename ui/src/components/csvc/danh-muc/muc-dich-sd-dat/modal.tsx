...
import { handleCreateOrUpdateMucdichsudungdat } from '@/app/(main)/quan-tri/csvc/danh-muc/muc-dich-sd-dat/actions';
...
interface IProps {
    ...
    dataUpdate: null | IMucdichsudungdat;
    setDataUpdate: (value: null | IMucdichsudungdat) => void;
}

const Context = React.createContext({ name: 'Default' });

const MucdichsudungdatModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateMucdichsudungdat(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm mục đích sử dụng đất' : 'Cập nhật mục đích sử dụng đất'}
...
export default MucdichsudungdatModal;
