...
import { handleDeleteMucdichsudungdat, handleDeleteMucdichsudungdatMany } from '@/app/(main)/quan-tri/csvc/danh-muc/muc-dich-sd-dat/actions';
import MucdichsudungdatModal from '@/components/csvc/danh-muc/muc-dich-sd-dat/modal';
import ModalImport from '@/components/csvc/danh-muc/muc-dich-sd-dat/modal.import';
...
const TableMucdichsudungdat = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | IMucdichsudungdat>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/muc-dich-sd-dat?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteMucdichsudungdatMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<IMucdichsudungdat> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách mục đích sử dụng đất</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="muc-dich-sd-dat.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <MucdichsudungdatModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableMucdichsudungdat;
