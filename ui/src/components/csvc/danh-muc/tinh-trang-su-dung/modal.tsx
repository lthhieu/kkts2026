...
import { handleCreateOrUpdateTinhtrangsudung } from '@/app/(main)/quan-tri/csvc/danh-muc/tinh-trang-su-dung/actions';
...
interface IProps {
    ...
    dataUpdate: null | ITinhtrangsudung;
    setDataUpdate: (value: null | ITinhtrangsudung) => void;
}

const Context = React.createContext({ name: 'Default' });

const TinhtrangsudungModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateTinhtrangsudung(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm tình trạng sử dụng' : 'Cập nhật tình trạng sử dụng'}
...
export default TinhtrangsudungModal;
