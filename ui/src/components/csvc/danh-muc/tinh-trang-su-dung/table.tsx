...
import { handleDeleteTinhtrangsudung, handleDeleteTinhtrangsudungMany } from '@/app/(main)/quan-tri/csvc/danh-muc/tinh-trang-su-dung/actions';
import TinhtrangsudungModal from '@/components/csvc/danh-muc/tinh-trang-su-dung/modal';
import ModalImport from '@/components/csvc/danh-muc/tinh-trang-su-dung/modal.import';
...
const TableTinhtrangsudung = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ITinhtrangsudung>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/tinh-trang-su-dung?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteTinhtrangsudungMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ITinhtrangsudung> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách tình trạng sử dụng</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="tinh-trang-su-dung.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <TinhtrangsudungModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableTinhtrangsudung;
