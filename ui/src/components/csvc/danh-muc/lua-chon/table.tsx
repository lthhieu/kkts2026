...
import { handleDeleteLuachon, handleDeleteLuachonMany } from '@/app/(main)/quan-tri/csvc/danh-muc/lua-chon/actions';
import LuachonModal from '@/components/csvc/danh-muc/lua-chon/modal';
import ModalImport from '@/components/csvc/danh-muc/lua-chon/modal.import';
...
const TableLuachon = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ILuachon>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/lua-chon?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteLuachonMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ILuachon> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="lua-chon.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <LuachonModal setStatus={setStatus} status={status} access_token={access_token}
