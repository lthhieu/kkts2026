...
import { handleDeleteHinhthucsohuu, handleDeleteHinhthucsohuuMany } from '@/app/(main)/quan-tri/csvc/danh-muc/hinh-thuc-so-huu/actions';
import HinhthucsohuuModal from '@/components/csvc/danh-muc/hinh-thuc-so-huu/modal';
import ModalImport from '@/components/csvc/danh-muc/hinh-thuc-so-huu/modal.import';
...
const TableHinhthucsohuu = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | IHinhthucsohuu>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/hinh-thuc-so-huu?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteHinhthucsohuuMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<IHinhthucsohuu> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách hình thức sở hữu</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="hinh-thuc-so-huu.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <HinhthucsohuuModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableHinhthucsohuu;
