...
import { handleCreateOrUpdateHinhthucsohuu } from '@/app/(main)/quan-tri/csvc/danh-muc/hinh-thuc-so-huu/actions';
...
interface IProps {
    ...
    dataUpdate: null | IHinhthucsohuu;
    setDataUpdate: (value: null | IHinhthucsohuu) => void;
}

const Context = React.createContext({ name: 'Default' });

const HinhthucsohuuModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateHinhthucsohuu(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm hình thức sở hữu' : 'Cập nhật hình thức sở hữu'}
...
export default HinhthucsohuuModal;
