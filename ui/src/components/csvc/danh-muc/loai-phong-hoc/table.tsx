...
import { handleDeleteLoaiphonghoc, handleDeleteLoaiphonghocMany } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-phong-hoc/actions';
import LoaiphonghocModal from '@/components/csvc/danh-muc/loai-phong-hoc/modal';
import ModalImport from '@/components/csvc/danh-muc/loai-phong-hoc/modal.import';
...
const TableLoaiphonghoc = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ILoaiphonghoc>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/loai-phong-hoc?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteLoaiphonghocMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ILoaiphonghoc> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách loại phòng học</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="loai-phong-hoc.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <LoaiphonghocModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableLoaiphonghoc;
