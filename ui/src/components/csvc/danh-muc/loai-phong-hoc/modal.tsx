...
import { handleCreateOrUpdateLoaiphonghoc } from '@/app/(main)/quan-tri/csvc/danh-muc/loai-phong-hoc/actions';
...
interface IProps {
    ...
    dataUpdate: null | ILoaiphonghoc;
    setDataUpdate: (value: null | ILoaiphonghoc) => void;
}

const Context = React.createContext({ name: 'Default' });

const LoaiphonghocModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateLoaiphonghoc(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm loại phòng học' : 'Cập nhật loại phòng học'}
...
export default LoaiphonghocModal;
