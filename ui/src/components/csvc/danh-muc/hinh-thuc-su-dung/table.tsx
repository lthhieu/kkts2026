...
import { handleDeleteHinhthucsudung, handleDeleteHinhthucsudungMany } from '@/app/(main)/quan-tri/csvc/danh-muc/hinh-thuc-su-dung/actions';
import HinhthucsudungModal from '@/components/csvc/danh-muc/hinh-thuc-su-dung/modal';
import ModalImport from '@/components/csvc/danh-muc/hinh-thuc-su-dung/modal.import';
...
const TableHinhthucsudung = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | IHinhthucsudung>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/hinh-thuc-su-dung?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteHinhthucsudungMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<IHinhthucsudung> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách hình thức sử dụng</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="hinh-thuc-su-dung.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <HinhthucsudungModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableHinhthucsudung;
