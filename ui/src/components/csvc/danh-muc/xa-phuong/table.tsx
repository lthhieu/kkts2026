...
import { handleDeleteXaphuong, handleDeleteXaphuongMany } from '@/app/(main)/quan-tri/csvc/danh-muc/xa-phuong/actions';
import XaphuongModal from '@/components/csvc/danh-muc/xa-phuong/modal';
import ModalImport from '@/components/csvc/danh-muc/xa-phuong/modal.import';
...
const TableXaphuong = (props: IProps) => {
    const { data, access_token, meta, user, tinhthanhpho } = props;
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isModalImportOpen, setIsModalImportOpen] = useState(false);
    const [status, setStatus] = useState('');
    const [loading, setLoading] = useState(false);
    const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
    const [dataUpdate, setDataUpdate] = useState<null | IXaphuong>(null);
    const router = useRouter();
    const [messageApi, contextHolder] = message.useMessage();
    const [api, contextHolderNotification] = notification.useNotification();
    const contextValue = useMemo(() => ({ name: 'Ant Design' }), []);
    const [selectedName, setSelectedName] = useState<string | undefined>(undefined);
    const [selectedTinhthanhpho, setSelectedTinhthanhpho] = useState<string | undefined>(undefined);
    const [mounted, setMounted] = useState(false);
    useEffect(() => setMounted(true), []);

    const deleteItem = async (_id: string) => {
        const res = await handleDeleteXaphuong(_id, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };

    const columns: TableProps<IXaphuong>['columns'] = [
        {
            title: 'Tên',
            dataIndex: 'name',
            key: 'name',
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        if (selectedTinhthanhpho) params.set('tinhthanhpho', selectedTinhthanhpho);
        params.set('current', current.toString());
        params.set('pageSize', pageSize.toString());
        router.push(`/quan-tri/csvc/danh-muc/xa-phuong?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteXaphuongMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="xa-phuong.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            {canReadDanhmuc(user ?? {} as IUser) && (
                <Space style={{ marginBottom: 16 }}>
                    ...
                    <Select
                        ...
                        onChange={(val) => setSelectedTinhthanhpho(val)}
                        onClear={() => setSelectedTinhthanhpho(undefined)}
                        options={tinhthanhpho.map((t) => ({ label: t.name, value: t._id }))}
                        showSearch={{ optionFilterProp: 'label' }}
                    />
                    <Button icon={<ClearOutlined />} onClick={() => { setSelectedName(undefined); setSelectedTinhthanhpho(undefined); }}>Xóa bộ lọc</Button>
