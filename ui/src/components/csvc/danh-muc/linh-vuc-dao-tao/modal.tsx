...
import { handleCreateOrUpdateLinhvucdaotao } from '@/app/(main)/quan-tri/csvc/danh-muc/linh-vuc-dao-tao/actions';
...
interface IProps {
    ...
    dataUpdate: null | ILinhvucdaotao;
    setDataUpdate: (value: null | ILinhvucdaotao) => void;
}

const Context = React.createContext({ name: 'Default' });

const LinhvucdaotaoModal = (props: IProps) => {
    ...
    const onFinish = async (values: { name: string }) => {
        const response = await handleCreateOrUpdateLinhvucdaotao(values, access_token ?? '', status, dataUpdate);
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal title={status === 'CREATE' ? 'Thêm lĩnh vực đào tạo' : 'Cập nhật lĩnh vực đào tạo'}
...
export default LinhvucdaotaoModal;
