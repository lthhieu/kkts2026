...
import { handleDeleteLinhvucdaotao, handleDeleteLinhvucdaotaoMany } from '@/app/(main)/quan-tri/csvc/danh-muc/linh-vuc-dao-tao/actions';
import LinhvucdaotaoModal from '@/components/csvc/danh-muc/linh-vuc-dao-tao/modal';
import ModalImport from '@/components/csvc/danh-muc/linh-vuc-dao-tao/modal.import';
...
const TableLinhvucdaotao = (props: IProps) => {
    ...
    const [dataUpdate, setDataUpdate] = useState<null | ILinhvucdaotao>(null);
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/danh-muc/linh-vuc-dao-tao?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteLinhvucdaotaoMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    const rowSelection: TableRowSelection<ILinhvucdaotao> = { selectedRowKeys, onChange: onSelectChange };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách lĩnh vực đào tạo</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadDanhmuc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="linh-vuc-dao-tao.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateDanhmuc(user ?? {} as IUser) && (
            ...
            />
            <LinhvucdaotaoModal setStatus={setStatus} status={status} access_token={access_token}
...
export default TableLinhvucdaotao;
