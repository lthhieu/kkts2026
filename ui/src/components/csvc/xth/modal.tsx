...
import { handleCreateOrUpdateXth } from '@/app/(main)/quan-tri/csvc/xth/actions';
...
interface IProps {
    ...
    dataUpdate: null | IXth;
    setDataUpdate: (value: null | IXth) => void;
    ctk: ICtk[];
...
const XthModal = (props: IProps) => {
    ...
    const onFinish = async (values: any) => {
        const response = await handleCreateOrUpdateXth(values, access_token ?? '', status, dataUpdate);
        if (response.data) {
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal
                title={status === 'CREATE' ? 'Thêm xưởng thực hành' : 'Cập nhật xưởng thực hành'}
