...
import ModalImport from '@/components/csvc/xth/modal.import';
...
const TableXth = (props: IProps) => {
    const { data, access_token, meta, user, ctk, linhvucdaotao } = props;
    ...
    const columns: TableProps<IXth>['columns'] = [
        {
    ...
    const handleOnChangePage = (current: number, pageSize: number) => {
        ...
        router.push(`/quan-tri/csvc/xth?${params.toString()}`);
    };
    ...
    const deleteMany = async (ids: string[]) => {
        const res = await handleDeleteXthMany(ids, access_token);
        if (!res.data) api.error({ title: 'Có lỗi xảy ra', description: res.message, placement: 'topRight' });
        else messageApi.success(res.message);
    };
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Flex style={{ marginBottom: 16 }} justify="space-between" align="center">
                <h2>Danh sách xưởng thực hành</h2>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {mounted && canReadCsvc(user ?? {} as IUser) && (
                        <Button type="primary" icon={<CloudDownloadOutlined />}>
                            <CSVLink data={data} filename="xth.csv" headers={headers} separator=";">Export</CSVLink>
                        </Button>
                    )}
                    {canCreateCsvc(user ?? {} as IUser) && (
