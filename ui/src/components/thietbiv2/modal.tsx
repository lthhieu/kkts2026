"use client"
import { Modal, Form, Input, message, Select, Row, Col, InputNumber, Typography, notification, Space, Switch } from 'antd';
import React, { useEffect, useMemo, useState } from 'react';
import { statusArr, typeArr } from '@/components/thietbiv2/table';
import { handleCreateOrUpdateThietbiV2 } from '@/app/(main)/quan-tri/thiet-biv2/actions';
...
const DeviceV2Modal = (props: IProps) => {
    const { setIsModalOpen, isModalOpen, setStatus, status, access_token, setDataUpdate, dataUpdate, rooms, units, email, availableChildren } = props
    ...
    const onFinish = async (values: IDeviceV2) => {
        const { name, description, usedLocation, usedYear, soKeToan, kiemKe, chenhLech, chatLuongConLai, note, trongSoChatLuong, type, unit, status: statusDevice, childrenIds } = values
        const data = { name, description, usedLocation, usedYear, soKeToan, kiemKe, chenhLech, chatLuongConLai, note, trongSoChatLuong, type, unit, parent, statusDevice, childrenIds }

        const response = await handleCreateOrUpdateThietbiV2(data, access_token ?? '', status, dataUpdate)
