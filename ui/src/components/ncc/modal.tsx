"use client"
import { handleCreateOrUpdateNcc } from '@/app/(main)/quan-tri/ncc/actions';
...
const NccModal = (props: IProps) => {
    ...
    const onFinish = async (values: IUnit) => {
        const { name } = values
        const data = { name }
        const response = await handleCreateOrUpdateNcc(data, access_token ?? '', status, dataUpdate)
    ...
    return (
        <Context.Provider value={contextValue}>
            ...
            <Modal
                title={status === "CREATE" ? "Thêm Nhà cung cấp" : "Cập nhật Nhà cung cấp"}
